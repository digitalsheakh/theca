...
export default function VehicleLookup({ darkMode = false, compact = false }: VehicleLookupProps) {
 ...
 const borderColor = darkMode ?'border-orange-600/30' :'border-gray-300';
 
 return (
 <div className={`${bgColor} ${textColor} ${compact ?'p-4' :'p-8'} shadow-lg`}>
 ...
 className="w-full py-3 px-4 bg-yellow-400 text-black font-bold placeholder-black/70 focus:outline-none uppercase"
 maxLength={10}
 />
 {error && (
 ...
 type="submit"
 disabled={isLoading}
 className={`w-full py-3 px-4 ${darkMode ?'bg-[#f56e13]' :'bg-[#c40b0b]'} hover:bg-opacity-90 text-white font-bold transition-colors duration-200 relative overflow-hidden`}
 >
 {isLoading ? (
