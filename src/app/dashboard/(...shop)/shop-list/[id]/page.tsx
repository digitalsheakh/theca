...
`}</style>
 ...
 className="w-full px-5 py-3 bg-white border border-orange-600/30 focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-transparent text-black placeholder-gray-500 transition-all duration-200 disabled:opacity-70 disabled:cursor-not-allowed"
 ...
 alt={`Preview ${index + 1}`} 
 className="h-24 w-full object-cover border border-orange-500/20"
 />
 <button
 type="button"
 onClick={() => removeImage(index)}
 className="absolute top-1 right-1 bg-orange-600 text-white p-1 opacity-0 group-hover:opacity-100 transition-opacity hover:bg-orange-700 disabled:opacity-50"
 disabled={isSubmitting}
 >
 ...
 {typeof preview !=='string' && (
 <span className="absolute bottom-1 left-1 bg-orange-500 text-white text-xs px-1">
 Existing
 </span>
 )}
 </div>
